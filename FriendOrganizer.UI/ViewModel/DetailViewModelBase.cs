...
namespace FriendOrganizer.UI.ViewModel
{
    public abstract class DetailViewModelBase : ViewModelBase, IDetailViewModel
    {
        ...
        {
            EventAggregator.GetEvent<AfterCollectionSavedEvent>().Publish(new
        ...
        protected async Task SaveWithOptimisticConcurrentcyAsync(Func<Task> saveFunc, Action afterSaveAction)
        {
            ...
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                var result = await MessageDialogService.ShowOkCancelDialogAsync("The entity has been changed in " +
                    "the mean time by another user. Click OK to save your changes, click Cancel " +
                    "to reload the entity from the database", "Question");

                if (result == MessageDialogResult.Ok)
                {
                    //Update database entry with entered values
